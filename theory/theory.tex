...
Each contribution $V^{MTP} (n_i)$ in the potential energy $E^{MTP}$
expands through a set of basis functions.
\begin{align}
	V^{MTP} (n_i)
	= \sum_{\alpha=1}^{N_{lin}} \xi_{\alpha} B_{\alpha} (n_i)
\end{align}
where $B_{\alpha}$ are the MTP basis functions,
$\xi_{\alpha}$ are the linear parameters to be found,
and $N_{lin}$ is the number of these parameters.
