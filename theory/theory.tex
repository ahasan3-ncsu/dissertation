...
is the sum of contributions $V (n_i)$ of atomic neighborhoods $n_i$
for $N$ atoms.
\begin{align}
...
The angular part $r_{ij}^{\otimes \nu}$ contains angular information
about the neighborhood $\nu$.
Here, the symbol '$\otimes$' denotes the outer product of vectors,
and, thus, the angular part is a tensor of rank $\nu$.

By definition, the MTP basis function $B_{\alpha}$ is a contraction
of one or more moment tensor descriptors, yielding a scalar, e.g.
\begin{align}
	M_{1,0}, (M_{1,2} M_{0,1}) \cdot M_{0,1}, M_{1,2} : M_{3,2}, \ldots,
\end{align}
where '$\cdot$' is an inner product of vectors,
and '$:$' is a Frobenius product of matrices.
Each basis function is invariant to
atomic permutations, rotations, and reflections.
In order to construct the basis functions $B_{\alpha}$,
and in turn, determine a particular functional form of MTP,
\textit{level} of moments $M_{\mu, \nu}$ and basis functions $B_{\alpha}$
are defined:
\begin{align}
	\lev M_{\mu, \nu}
		&= 2 + 4 \mu + \nu \\
	\lev B_{\alpha}
		&= \lev \prod_{p=1}^P M_{\mu_p, \nu_p}
		= \sum_{p=1}^P (2 + 4 \mu_p + \nu_p)
\end{align}
A set of MTP basis functions,
and thus, a particular form of MTP
depends on the maximum allowed level $\lev_{max}$,
which is also called the level of MTP.
The set of basis functions included in the MTP
are the ones with $\lev B_{\alpha} \leq \lev_{max}$
\cite{novikov2020mlip, podryabinkin2023mlip}.
