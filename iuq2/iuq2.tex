...
The NN model performed similarly to the GP model on the test split,
...
Since the accuracies of the GP and NN models are comparable,
we chose to utilize the GP model for the rest of the IUQ process
due to its inherent ability to quantify uncertainty.
...
The acceptance rate of the MCMC sampling was about $25 \%$ for both chains,
...
For all $9$ parameters, the ESS is at least $3,000$,
...
for \texttt{FaceCovMax}, \texttt{SwellLink},
\texttt{rResolBulk}, and \texttt{vResol}
...
\texttt{FaceCovMax}, \texttt{SwellLink}, \texttt{rResolBulk},
...
These seemingly noninfluential parameters
might be irrelevant for fuel swelling at specific fission densities
...
by propagating the posterior samples through the GP surrogate models.
...
The density at low $F_d$ is significantly narrower than that of the observation.
...
has a mean absolute error of about $0.19 \%$,
...
Although the distributions from the forward propagation
does not exactly match that of the observation, especially at low $F_d$,
