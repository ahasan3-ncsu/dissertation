...
\begin{frame}{Introduction}
	...
	\column{0.5\textwidth}
		\begin{itemize}
			...
			\item U$_3$Si$_2$ is the qualified LEU fuel
				with the highest uranium loading.
	...
	\column{0.5\textwidth}
		\begin{itemize}
			...
			\item U-10Mo monolithic fuel design involves
				a solid U-10Mo fuel meat bonded to Al-6061 cladding
	...
	\column{0.5\textwidth}
		\begin{itemize}
			...
			\item To account for it in the fuel design,
				fuel swelling needs to be predictable
...
\begin{frame}{Grain boundary (GB) diffusion in $\gamma$U-Mo}
	\begin{itemize}
		...
		\item Accurate calculation of fuel swelling requires
			...
			the fission gas (such as Xe) trapping rate of the bubbles.
...
\begin{frame}{Xe gas bubble re-solution}
	\begin{itemize}
		\item Xe gas bubbles act as sinks for individual Xe atoms,
		...
		\item There is no physical model for the Xe re-solution rate in U-Mo yet.
	\end{itemize}
\end{frame}
...
\begin{frame}{Other fission-gas-behavior parameters}
	\begin{itemize}
		...
		\item The fission-gas-behavior parameters used in
			the gas release and swelling subroutine (GRASS) module of DART
	...
	\column{0.5\textwidth}
		\begin{figure}[ht]
			...
			\caption{
				...
				(Beeler et al. {\color{blue}
				\url{https://doi.org/10.1016/j.jnucmat.2018.05.007}}.)
			}
		\end{figure}
	\end{columns}
\end{frame}
	...
	\column{0.5\textwidth}
		\begin{figure}[ht]
			...
			\caption{
				...
				(Smirnova et al. {\color{blue}
				\url{https://doi.org/10.1134/S0031918X1503014X}}.
				Park et al. {\color{blue}
				\url{https://doi.org/10.1016/j.jnucmat.2022.154137}}.)
			}
		\end{figure}
	\end{columns}
\end{frame}
...
\begin{frame}{Fission fragment and bubble interactions}
	...
	\column{0.5\textwidth}
		\begin{itemize}
			\item A specific 3D geometry \texttt{SPHEREINCUBOID}
				has been implemented in RustBCA
				to allow the simulation of gas bubbles in solid materials.
				{\color{blue} \url{https://github.com/ATM-Jahid/RustBCA}}.
...
\begin{frame}{Fission-gas-behavior parameters}
	...
	\begin{itemize}
		\item $3,200$ different combinations of the parameters
			were used in DART to simulate swelling
			for each operational condition of the experiments.
	...
	\column{0.5\textwidth}
		\begin{itemize}
			...
			\item Many machine learning models,
				such as Gaussian processes (GPs) and neural networks (NNs),
...
\begin{frame}{DART surrogates}
	...
	\column{0.5\textwidth}
		\begin{itemize}
			\item GPs with a Mat\'ern kernel was found to perform the best
				with $R^2$ scores of $0.99$ and $0.97$,
				and mean absolute errors of $0.19 \%$ and $1.94 \%$,
				for emulating low and high $F_d$, respectively.
			\item The NN models have $4$ hidden layers with $250$ neurons each,
				and have $R^2$ scores of $0.98$ and $0.98$,
