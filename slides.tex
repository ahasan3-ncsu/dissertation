...
\begin{frame}{Introduction (cont.)}
	...
	\column{0.5\textwidth}
		\begin{itemize}
			...
			\item Before the monolithic design,
				the U-Mo dispersion fuel had been tested.
...
\begin{frame}{Grain boundary (GB) diffusion in $\gamma$U-Mo}
	...
	The diffusion coefficients of the relevant species
...
\begin{frame}{Simulation setup}
	...
	\column{0.5\textwidth}
		\begin{itemize}
			\item The dimensions of the supercells are at least
...
\begin{frame}{Xe gas bubble re-solution}
	Xe gas bubbles act as a sink for individual Xe atoms,
...
\begin{frame}{Quantifying unknown DART parameters}
	The key challenge in simulating fission gas swelling
...
\begin{frame}{Machine-learned interatomic potential (MLIP)}
	\begin{columns}
	\column{0.5\textwidth}
		\begin{itemize}
			\item Currently, MD simulations of the fuel are limited to
				the use of only three elements: U, Mo, and Xe.
			\item This limits the number of complex scenarios we can simulate.
			\item The figure shows our attempt
				to understand blistering mechanism using MD,
				where we used Mo as a liner.
			\item We want to use elements like Al or Zr in simulations.
				Thus, we are trying to build MLIPs.
		\end{itemize}
	\column{0.5\textwidth}
		\begin{figure}[ht]
			\centering
			\includegraphics[width=5cm, angle=90]{sfigs/blistering_md.png}
			\caption{
				Simulation of blistering.
				Red, blue, and yellow spheres represent U, Mo, and Xe.
			}
		\end{figure}
	\end{columns}
\end{frame}
...
\begin{frame}{Moment tensor potential (MTP)}
	\begin{itemize}
		\item Various U and Mo phases and alloys are simulated with AIMD
			using VASP.
			The simulated systems are:
			\begin{itemize}
				\item orthorombic $\alpha$U
				\item bcc $\gamma$U
				\item bcc Mo
				\item tetragonal U$_2$Mo
				\item bcc $\gamma$U-10Mo
			\end{itemize}
		\item The resulting data is then used with MLIP-3 package
			to train an MTP.
		\item So far, the potential only has two elements: U and Mo.
		\item The LAMMPS package is utilized
			along with an MLIP-3 interface layer
			to run MD simulations with the trained MTP.
	\end{itemize}
\end{frame}
