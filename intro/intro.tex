...
that supply high neutron and/or gamma radiation
for research into basic/applied sciences \cite{martens1967}.
These reactors operate at lower temperature and pressure than power reactors
and are often simpler in design \cite{couturier2019}.
However, RRs normally require fuel
that is much more enriched in U-235 than the fuel used in power reactors.
Generally, highly enriched uranium (HEU) fuels are defined
as those containing greater than 20\% U-235 content,
whereas low enriched uranium (LEU) fuels contains less than 20\% U-235 content.
RRs have historically employed HEU for their operation \cite{keiser2003}.

The United States High-Performance Research Reactor (USHPRR) program
aims to replace HEU fuels in high-power research reactors with LEU fuels.
This transition is driven by the need
to move away from weapons-grade nuclear material
and reduce proliferation risks.
To compensate for the lower enrichment,
a higher-density uranium fuel is required
to sustain sufficient power and flux levels in research reactors
\cite{snelgrove1997, wilson2020}.

High-performance research reactors require fuel
capable of operating at low temperatures
while maintaining high fission density at high specific power.
Peak fuel centerline temperature in these reactors
is typically less than $250^{\circ}$C,
with fuel in most reactors operating at significantly lower temperatures
\cite{kim2011}.
The fission density for a candidate fuel
falls between $3 \times 10^{21}$ and $6 \times 10^{21}$ fission/cm$^3$.

% historic RR fuels here and why they failed

Only a few fuels, such as stable $\gamma$U alloys, combine high uranium density
with stable behavior at such high burnup levels \cite{meyer2014}.
The $\gamma$ phase of uranium
...
% dispersion fuels too
Consequently, the USHPRR program is developing fuel designs
using $\gamma$U-Mo as a monolithic fuel foil,
incorporating a zirconium diffusion barrier
within aluminum cladding \cite{robinson2009, cole2016, miller2021}.

There exists a knowledge gap in the material properties of $\gamma$U-Mo,
...
of less than $2.5$--$3.5 \times 10^{21}$ fissions/cm$^3$
or a burnup of $6$--$7.2\%$ fission per initial metal atoms (FIMA)
and temperatures typically below $250^{\circ}$,
...
reflecting an evolution of the nanobubble.

The initial swelling is primarily attributed to
the accumulation of fission products
either in solid solution within the fuel lattice or within the ordered GBS.
As irradiation continues and the fuel reaches higher fission densities,
a significant microstructural transformation---grain refinement---occurs.
This leads to the development of the high burnup structure (HBS).
...
where defect annealing is limited
due to progressive accumulation of radiation defects.
...
The increased grain boundary area per unit volume in the recrystallized fuel
enhances fission gas bubble swelling.
Three classes of porosity are observed in irradiated $\gamma$U-Mo fuel:
small gas bubbles within fuel particles and interaction layers,
as-fabricated pores,
and large pores at the interaction layer-matrix interface,
all of which can reduce thermal conductivity
and contribute to fuel plate swelling.
...
The relative rates of re-solution affect the overall size and number density
...
In the literature up to this point, atomistic simulations have been used
...
a lattice atom is endowed with high kinetic energy to emulate a PKA.
...
\subsection{Unknown Fission-Gas-Behavior Parameters}
...
Based on the identified knowledge gaps in $\gamma$U-Mo, the following
objectives will be pursued utilizing various computational methods:

\begin{itemize}
\item
The diffusivities of U, Mo, and Xe in $\gamma$U-Mo GBs
will be computed using classical MD simulations.
Different types of symmetric tilt, asymmetric tilt, and twist GBs
will be utilized for the calculations.
The effect of fuel composition on diffusion will also be examined.
...
\item
We will utilize MD simulations to investigate
...
We will also calculate the electronic stopping power
...
Finally, we will evaluate an overall re-solution rate of Xe gas bubbles
as a function of bubble radius, bubble pressure, and fission rate.

\item
Inverse uncertainty quantification wil be performed
to obtain probability distributions
of a few fission-gas-behavior parameters used in DART.
First, the data compiled from some DART calculations will be visualized.
Afterward, surrogate models will be built using the data.
A variety of linear and non-linear models will be used for this purpose.
Afterward, Bayesian inference methods will be applied
to find the posterior probability distributions of the DART parameters.
Finally, the surrogate model will be used again
along with the parameter posteriors
