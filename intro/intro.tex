...
In general, RRs operate at relatively low temperatures and pressures
...
\begin{figure}
	...
	\caption{
		Uranium density of research reactor fuel vs. time of first use.
		\textit{Reproduced from Jamison et al. \cite{jamison2022}.}
	}
	\label{fig:umo_design}
\end{figure}
...
Alloying $\gamma$-U with molybdenum produces a metastable $\gamma$ phase,
...
(Figure \ref{fig:umo_design}) \cite{jamison2022, mishra2023}.
...
Under irradiation, $\gamma$U-Mo fuel undergoes substantial
microstructural evolution that impact its performance.
...
to a multitude of much smaller grains
(Figure \ref{fig:recryst}) \cite{leenaers2016}.
...
\begin{figure}[ht]
	...
	\caption{
		Scanning electron micrographs showing evolution of fuel microstructure.
		With increasing burnup, large grains convert to many smaller grains.
...
The fuel swelling model of $\gamma$U-Mo needs the GB diffusion coefficients
to advise the GB enhancement factor \cite{cui2015, annualreport2021},
and the phase-field models of gas bubble evolution in $\gamma$U-Mo
require the diffusivity of gas atoms
both in the bulk and along grain boundaries \cite{hu2021, annualreport2021}.
...
and bcc tungsten \cite{fu2021}, etc.

\subsection{Fission Gas Bubble Re-Solution}

Understanding the progression of Xe gas bubbles in the fuel
is of paramount importance for optimizing reactor performance and safety.
These Xe gas bubbles act as a sink for individual Xe atoms,
trapping them and causing the bubbles to grow after absorption.
Under irradiation, the Xe atoms in the gas bubbles are reintroduced
...
\subsection{Fission-Gas-Behavior Parameters in DART}

The current understanding of the U-Mo fuel behavior is encapsulated
in the rate-theory-based computational code
Dispersion Analysis Research Tool (DART),
...
in dispersion fuels (U$_3$Si$_2$-Al and U-Mo/Al) \cite{rest1999, ye2015}.
In 2018, a computational module for U-10Mo was added
...
The output from DART includes total porosity
as well as intra- and inter-granular bubble size distributions
at various locations (bulk, grain face, grain edge, and grain corner).
The results from DART have been validated against experimental data,
and DART has been used to study the effects of various operational and
microstructural parameters on fuel swelling \cite{annualreport2021, ye2023}.

GRASS assumes bubbles nucleate within the grains
as fission product gases emerge.
These gas atoms and bubbles subsequently diffuse to the GBs
induced by concentration and temperature gradients.
This migration process is also affected by the rates of
intragranular trapping and irradiation-dependent re-solution.
When the grain face coverage by intergranular bubbles reaches saturation,
gas atoms move to the grain edges.
Gas atoms stay trapped on grain faces
till interconnection of the bubbles occurs,
allowing the gas atoms to move to the grain corners \cite{ye2023}.
A schematic of this process is shown in Figure \ref{fig:dart}.
Simulating the whole process requires the knowledge of
many microstructural properties of the fuel.
Two of the key properties,
such as the GB diffusion coefficients and the Xe gas bubble re-solution rate,
are already discussed in the previous sections.
Other parameters include bubble formation probability (\texttt{StickProb}),
threshold areal coverage of grain faces needed for saturation
(\texttt{FaceCovMax}),
threshold fuel swelling to enable gas migration from grain faces to corners
(\texttt{SwellLink}), etc. \cite{annualreport2022, annualreport2023}

\begin{figure}[h]
	\centering
	\includegraphics[trim={0 14cm 0 0}, clip, width=16cm]
...
The major challenge in simulating fission gas swelling
with a mechanistic model such as DART
is obtaining the microstructural properties related to gas bubble behavior,
...
Based on the identified knowledge gaps, this dissertation pursues
the evaluation of atomistic and microstructural properties
...
are used for global sensitivity analysis.
